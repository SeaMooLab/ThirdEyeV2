...
const assets = ["CHANGELOG.md", "LICENSE", "manifest.json", "pack_icon.jpg", "README.md"];
...
function createDistributionArchive(outputFilePath, addonDir) {
    ...
    const result = runCommand(sevenZipPath, ["a", "-tzip", outputFilePath, "build/addon/behavior_packs"], { cwd: addonDir });
...
(async () => {
    ...
    if (!isServerMode) {
        ...
        const outputFileName = `ThirdEye-v${packageVersion}.${process.argv.includes("--mcpack") ? "mcaddon" : "zip"}`;
